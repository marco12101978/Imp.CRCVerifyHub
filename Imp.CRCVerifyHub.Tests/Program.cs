...
namespace Imp.CRCVerifyHub.Tests
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($" CHECKSUN -> {Crc16.Services.crcChecksumGenerator(algorithm: Crc16.Crc16Algorithm.Ccitt,bytes: buffer, start: 1,length: buffer.Length -1).ToString("X2").Insert(2, " ")}");
            ...
            Array.ForEach(buffer, b => Console.Write($"{b:X2} "));
            Console.WriteLine($" CHECKSUN -> {Crc16.Services.crcChecksumGenerator(Crc16.Crc16Algorithm.Ccitt, buffer).ToString("X2").Insert(2, " ")}");
