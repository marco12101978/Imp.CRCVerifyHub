...
{
    /// <summary>
    /// Classe para cálculo de CRC 16 usando o polinômio invertido x^16 + x^15 + x^2 + 1 (0xA001) com um valor CRC inicial de 0
    /// </summary>
    internal static class StandardCrc16
    {
        ...
        private const ushort StandardInitialValue = 0;
        ...
        internal static ushort Checksum(byte[] bytes, int start, int length, ushort initialValue = StandardInitialValue)
        {
            if (bytes == null) throw new ArgumentNullException(nameof(bytes));
            if (bytes.Length == 0) throw new ArgumentOutOfRangeException(nameof(bytes));
            if (start < 0 || start >= bytes.Length && length > 1) throw new ArgumentOutOfRangeException(nameof(start));
