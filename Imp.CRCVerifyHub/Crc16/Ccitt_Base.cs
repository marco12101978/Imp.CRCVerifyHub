...
namespace Imp.CRCVerifyHub.Crc16
{
    ...
    internal static class Ccitt_Base
    {
        // Tabela pré-calculada usando o polinômio x^16 + x^12 + x^5 + 1 / 0x1021

        internal static ushort[] Table { get; } = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
        ...
        internal static ushort Checksum(ushort initialValue, params byte[] bytes)
        {
            return Checksum(initialValue, bytes, 0, bytes?.Length ?? 0);
        }
        internal static ushort Checksum(ushort initialValue, byte[] bytes, int start, int length)
        {  
